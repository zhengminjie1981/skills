...
**问题**：SKILL.md 膨胀到 290 行，包含过多高级内容，影响 AI 友好性。
...
| 维度 | 之前 | 之后 |
|-----|------|------|
| SKILL.md 行数 | 290 行 | 151 行 |
| 入口内容 | 包含全局注册表、层级管理 8 场景详情 | 核心概念 + 按需加载指南 |
| 加载策略 | 实际全加载 | 显式触发器 |

### 新增：按需加载指南

在 SKILL.md 中添加了触发器表，指导 AI 何时读取详细文档：

```markdown
| 任务 / 场景 | 读取文档 |
|------------|---------|
| 首次构建索引 | references/core/workflow-spec.md |
| 检测到父子冲突 | references/advanced/hierarchy-management.md |
| 查看所有知识库 | references/advanced/global-registry-spec.md |
```
...
| 文件 | 变更 |
|-----|------|
| `SKILL.md` | 精简 48%，添加按需加载指南 |
| `README.md` | 更新 knowledge-index 描述 |
