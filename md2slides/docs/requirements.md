...
> 文档层级：L1 系统级
> 版本：v0.4 | 创建日期：2026-03-23 | 更新日期：2026-03-24
...
阶段一：内容策划（MD）
  用户提供原始材料/文档
    → AI 分析内容，规划演示结构
    → 识别数据内容 → 提取数据 → 保存为 CSV / Dataview 格式
    → 生成 MD 格式演示文稿（明确分页、每页内容）
        含数据页：指定图表类型、数据源文件、位置与尺寸参数
    → 用户确认或修改 MD 内容
...
- 嵌套列表不超过 3 层（视觉效果差）
- PDF 转换需要 playwright 环境（首次使用需安装 Chromium，约 200MB）
...
| v0.4 | 2026-03-24 | 新增内容变更需求：两种调整路径区分、MD 优先的内容变更工作流、样式保留策略 A | 新增数据可视化需求：数据提取存储（CSV/Dataview）、图表类型与参数标注、Chart.js 渲染、图表元素树支持 |
